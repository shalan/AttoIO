...
| H13 | **Further RAM downsize** — 512 B SRAM A | ☑ | Phase 0.9 — dropped second RAM128, leaving 1× RAM128 + 1× RAM32 (2 DFFRAMs total). `rtl/attoio_memmux.v` collapses to single-bank SRAM A; `rtl/attoio_macro.v` drops `u_sram_a1`; `sw/link.ld` RAM_A 1024 → 512. All TBs' `fw_image [0:255]`→`[0:127]`. `i2c_eeprom` (584 B) doesn't fit and is excluded from default regression — kept as archival. `ir_learn` trimmed (468 → 416 B) to fit. **Phase 0.9 standalone synth + STA:** 0.251 mm² (vs 0.399 in Phase 0.8, −37 %), Setup WNS @ 75 MHz = **+1.25 ns** (was +0.97), power 38.2 mW (was 73.3 mW @ 10 % activity). **Regression: 28/28 PASS** excluding `i2c_eeprom`. |
