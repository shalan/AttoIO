/******************************************************************************/
// attoio_macro_cfsram — AttoIO CFSRAM variant (4 KB CF_SRAM_1024x32).
//
// Sibling of attoio_macro (DFFRAM-based) with a hard-macro SRAM A:
//   - AttoRV32 core (RV32EC, ADDR_WIDTH=13)
//   - 1 × SRAM A (CF_SRAM_1024x32 hard macro)  → 4096 B of private SRAM
//   - 1 × SRAM B (32×32 DFFRAM)                → 128 B of host/IOP mailbox
//   - APB4 slave interface on the host side, 13-bit PADDR
//   - Memory mux + address decoder + SRAM B arbiter (attoio_memmux_cfsram)
...
//
// Memory map (from both IOP and APB views; ADDR_WIDTH = 13):
//   0x0000 – 0x0FFF  SRAM A (CF_SRAM_1024x32, 4 KB)
//   0x1400 – 0x147F  SRAM B mailbox (32×32 DFFRAM, 128 B)
//   0x1700 – 0x17FF  MMIO page (256 B)
//
// PINMUX semantics (2 bits per pad, in attoio_ctrl register @ APB 0x1710/0x1714):
...
module attoio_macro_cfsram #(
    ...
    attoio_apb_if #(.ADDR_WIDTH(13)) u_apb (
        .PCLK        (sysclk),
        .PRESETn     (rst_n),
        .PADDR       (PADDR),
        .PSEL        (PSEL),
    ...
    wire [12:0] core_pc_out;    /* ADDR_WIDTH = 13 */

    /* SRAM A banks */
    /* SRAM A (private, 512 B) */
    wire [9:0]  sram_a_a0;  wire [31:0] sram_a_di0;
    ...
    // ====================================================================
    // SRAM A — 4 KB CF_SRAM_1024x32 hard macro (sysclk)
    // Wrapped with cfsram_dffram_wrapper to keep the rest of the flow
    // speaking the familiar DFFRAM interface (CLK/EN0/WE0/A0/Di0/Do0).
    // ====================================================================
    cfsram_dffram_wrapper u_sram_a (
        .CLK (sysclk),
        .WE0 (sram_a_we0),
        .EN0 (sram_a_en0),
        .A0  (sram_a_a0),
    ...
    // ====================================================================
    // SRAM B — mailbox (sysclk, 128 B DFFRAM, unchanged from v1.x)
    // ====================================================================
    ...
    // ====================================================================
    // Memory mux — CFSRAM-specific (13-bit addressing, 4 KB SRAM A)
    // ====================================================================
    attoio_memmux_cfsram u_memmux (
        .sysclk      (sysclk),
        .clk_iop     (clk_iop),
        .rst_n       (rst_n),
        .iop_reset   (iop_reset),

        .core_addr   (core_mem_addr[12:0]),
